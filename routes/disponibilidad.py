...
def get_disponibilidad(tipo_vehiculo: int, tipo_tarifa: int):
    subquery = conn.execute((select(func.count()).select_from(registros).where((registros.columns.tipo_vehiculo == tipo_vehiculo) & (registros.columns.registro_activo == '1') & (registros.columns.en_parqueadero == '1') & (registros.columns.tipo_tarifa == tipo_tarifa))))
    if tipo_tarifa == 1 or tipo_tarifa == 2 or tipo_tarifa == 3:
        return conn.execute(select(((vehiculos.columns.capacidad/2)-subquery).label('disponibles'), ((select(func.count()).select_from(registros).where((registros.columns.tipo_vehiculo == tipo_vehiculo) & (registros.columns.registro_activo == '1') & (registros.columns.en_parqueadero == '1') & (registros.columns.tipo_tarifa == tipo_tarifa)))).label('ocupados')).select_from(vehiculos).where(vehiculos.columns.id_vehiculo == tipo_vehiculo)).first()
    elif tipo_tarifa == 4:
        return conn.execute(select(((vehiculos.columns.capacidad/2)-subquery).label('disponibles'), ((select(func.count()).select_from(registros).where((registros.columns.tipo_vehiculo == tipo_vehiculo) & (registros.columns.registro_activo == '1') & (registros.columns.en_parqueadero == '1') & (registros.columns.tipo_tarifa == tipo_tarifa)))).label('ocupados')).select_from(vehiculos).where(vehiculos.columns.id_vehiculo == tipo_vehiculo)).first()
    else:
